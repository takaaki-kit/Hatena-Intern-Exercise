...
function addLogTable(){
    var logData = document.getElementById("log-input");
    var target = document.getElementById("table-container");

    var parseData = parseLTSVLog(logData.innerHTML);
    createLogTable(target,parseData);

}

function load(){
    var target = document.getElementById("submit-button");
    target.addEventListener("click", addLogTable, false);
}
